...
LOG_MODULE_REGISTER(xg22_ek2710a, CONFIG_BOARD_XG22_EK2710A_LOG_LEVEL);

static int xg22_ek2710a_init_clocks(void);

static int xg22_ek2710a_init(void)
{
#ifdef CONFIG_SOC_GECKO_DEV_INIT
	xg22_ek2710a_init_clocks();
#endif
	...
	int ret = gpio_pin_configure_dt(&wake_up_gpio_dev, GPIO_OUTPUT_ACTIVE);
	if (ret < 0) {
		return ret;
	}
...
SYS_INIT(xg22_ek2710a_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
